//Anthony Carpenetti
//When executed, Euler8 finds the largest product of 13 consecutive integers stored as the String data

public class Euler8
{
   public static void main(String[] args)
   {
      int ansChoice = 0;
	  int ansSum = 0;
      int size = 13;
	  long ans = 1;
      String data = "";
      int answerChoicesCount = data.length() - size;
	  String[] choices = new String[answerChoicesCount];
      for(int i = 0;i < answerChoicesCount;i++)
	  {
		  choices[i] = data.substring(i, i + size);
		  int lastIndex = i + size - 1;
		  int sum = 0;
		  if(!data.substring(i, i + size).contains("0"))
		  {
			  for(int j = i;j < lastIndex;j++)
			  {
				  sum += Integer.parseInt(data.substring(j, j + 1));
			  }
			  
			  if(sum > ansSum)
			  {
				  ansSum = sum;
				  ansChoice = i;
			  }

		  }
	  }
	  
	  for(int i = 0;i < size;i++)
	  {
		  ans *= Integer.parseInt(choices[ansChoice].substring(i, i + 1));
	  }
	  System.out.println(ans);
	  /* for(int i = 0;i < answerChoicesCount;i++)
      {
         int lastIndex = i + size - 1;
         long product = 1;
         String debug = "";

         if(!data.substring(i, i + size).contains("0"))
         {
            for(int j = i;j < lastIndex;j++)
            {
               String numberAtIndexJ = data.substring(j, j + 1); //numberAtIndexJ will always be a single character String that isnt "0"
               product *= Long.parseLong(numberAtIndexJ);
            }
         }
         else
         {
            
         }
         if(product > ans)
         {
            ans = product;
         }
      }
      System.out.println(ans); */
   }
}
